...
build_with_cloud_build() {
    ...
    # NOTE: We use gcr.io/cloud-builders/docker instead of Kaniko because
    # Kaniko does not properly preserve ENTRYPOINT/CMD metadata from complex
    # base images like Cloud Workstations images, which breaks SSH.
    ...
    if [[ "$platforms" == *","* ]]; then
        ...
        # Generate Cloud Build config for multi-arch using Docker buildx
        cat > "$cloudbuild_file" << EOF
# Auto-generated Cloud Build configuration for multi-architecture build
# Generated by workstation-image.sh
# Uses Docker (not Kaniko) to preserve base image entrypoint metadata
...
EOF
        ...
        # NOTE: We don't specify --platform for single-arch builds because:
        # 1. Cloud Build workers are already linux/amd64
        # 2. Specifying --platform can cause issues with multi-arch base images
        cat > "$cloudbuild_file" << EOF
